...
describe("use()", () => {
    it("should register methods to the class", () => {
        ...
        expect(c.lighten(10).to("hsl")).toBe("hsl(50 50 60)");
        expect(c.darken(20).to("hsl")).toBe("hsl(50 50 30)");
    });

    it("should throw if called with no arguments", () => {
        expect(() => use()).toThrow("use() requires at least one plugin function.");
    });

    it("should throw if a non-function plugin is passed", () => {
        expect(() => use("notAFunction" as unknown as () => void)).toThrow(
            "Plugin at index 0 is not a function. Received: string"
        );
    });
